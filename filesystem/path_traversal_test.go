...
func TestEdgeCases(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			path:        baseDir,
			baseDir:     "",
			shouldPass:  false, // Changed to false since empty base dirs are now explicitly rejected for security
			description: "Empty base directory (should be rejected for security)",
		},
		{
			name:        "DotPath",
			path:        ".",
