...
func TestIsTextFile(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// Use the parent directory of the file as baseDir
			baseDir := filepath.Dir(tc.path)
			isText, err := IsTextFile(tc.path, baseDir)
			if tc.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestGatherLocalFiles(t *testing.T) {
	...
	files := map[string]string{
		...
		GlanceFilename: "glance file",                    // Should be ignored (glance output)
	...
	t.Run("Basic gathering with no ignore rules", func(t *testing.T) {
		...
		// Should not contain hidden, glance output, or nested files
		assert.NotContains(t, results, ".hidden.txt")
		assert.NotContains(t, results, GlanceFilename)
